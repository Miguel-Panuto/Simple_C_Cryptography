...
/*
	...
	confirma = confirmar se a chave � igual ao digitado no Descriptografar().
...
void Prga(){//Pseudo-random generation algorithm
	for(k = 0; k < tam_entrada ; k++){ //Fazendo a mensagem criptografada e embaralhando
		...
		j = (j + s[i])%256;
		TrocaVal(s, i, j);
		msgCrip[k] = (s[(s[i] + s[j])%256]) 3 msg[k]; // Modifica o estado e saida do byte, ^ porta lógica XOR
  }
}
//--------------------------------------------------------------------------------------------------------------------
...
void Descriptografar(){//Descriptografia
	...
	op = getch();
	setbuf(stdin, NULL);
	if (op != 's'){ // Caso o usuario digite s ele volta pro menu, qualquer outro o programa fecha
		clear();
		gotoxy(1,1);// muda o ponto do cursor
